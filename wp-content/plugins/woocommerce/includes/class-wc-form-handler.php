...
class WC_Form_Handler {
	...
	public static function save_account_details() {
		...
		$required_fields = apply_filters(
			...
			array(
				'account_first_name'   => __( 'Nom', 'woocommerce' ),
				'account_last_name'    => __( 'Prénom', 'woocommerce' ),
				'account_display_name' => __( 'Identifiant', 'woocommerce' ),
				'account_email'        => __( 'Email', 'woocommerce' ),
			)
		);
		...
		foreach ( $required_fields as $field_key => $field_name ) {
			if ( empty( $_POST[ $field_key ] ) ) {
				/* translators: %s: Field name. */
				wc_add_notice( sprintf( __( '%s est un champs requis.', 'woocommerce' ), '<strong>' . esc_html( $field_name ) . '</strong>' ), 'error', array( 'id' => $field_key ) );
			}
		}
		...
		if ( $account_email ) {
			...
			if ( ! is_email( $account_email ) ) {
				wc_add_notice( __( 'Merci de fournir une adresse mail valide.', 'woocommerce' ), 'error' );
		...
		if ( ! empty( $pass_cur ) && empty( $pass1 ) && empty( $pass2 ) ) {
			wc_add_notice( __( 'Merci de remplir tous les champs de mot de passe.', 'woocommerce' ), 'error' );
			$save_pass = false;
		} elseif ( ! empty( $pass1 ) && empty( $pass_cur ) ) {
			wc_add_notice( __( 'Merci de fournir votre mot de passe actuel.', 'woocommerce' ), 'error' );
			$save_pass = false;
		} elseif ( ! empty( $pass1 ) && empty( $pass2 ) ) {
			wc_add_notice( __( 'Merci de resaisir votre mot de passe.', 'woocommerce' ), 'error' );
			$save_pass = false;
		} elseif ( ( ! empty( $pass1 ) || ! empty( $pass2 ) ) && $pass1 !== $pass2 ) {
			wc_add_notice( __( 'Les mots de passe ne se correspondent pas.', 'woocommerce' ), 'error' );
			$save_pass = false;
		} elseif ( ! empty( $pass1 ) && ! wp_check_password( $pass_cur, $current_user->user_pass, $current_user->ID ) ) {
			wc_add_notice( __( 'Votre mot de passe actuel est incorrecte.', 'woocommerce' ), 'error' );
			$save_pass = false;
		}
		...
		if ( isset( $_POST['login'], $_POST['username'], $_POST['password'] ) && wp_verify_nonce( $nonce_value, 'woocommerce-login' ) ) {
			...
			try {
				...
				if ( empty( $creds['user_login'] ) ) {
					throw new Exception( '<strong>' . __( 'Error:', 'woocommerce' ) . '</strong> ' . __( 'L\'identifiant est requis.', 'woocommerce' ) );
				}
