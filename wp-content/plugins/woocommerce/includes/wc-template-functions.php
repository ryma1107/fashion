...
function wc_template_redirect() {
	...
	if ( is_page( wc_get_page_id( 'checkout' ) ) && wc_get_page_id( 'checkout' ) !== wc_get_page_id( 'cart' ) && WC()->cart->is_empty() && empty( $wp->query_vars['order-pay'] ) && ! isset( $wp->query_vars['order-received'] ) && ! is_customize_preview() && apply_filters( 'woocommerce_checkout_redirect_empty_cart', true ) ) {
		wc_add_notice( __( 'Le paiement est indisponible tant que votre panier est vide.', 'woocommerce' ), 'notice' );
...
if ( ! function_exists( 'woocommerce_catalog_ordering' ) ) {
	...
	function woocommerce_catalog_ordering() {
		...
		$catalog_orderby_options = apply_filters(
			...
			array(
				'menu_order' => __( 'Par défaut', 'woocommerce' ),
				'popularity' => __( 'Tri par popularité', 'woocommerce' ),
				'rating'     => __( 'Tri par classement', 'woocommerce' ),
				'date'       => __( 'Tri par le plus récent', 'woocommerce' ),
				'price'      => __( 'Tri par prix: Du min au max', 'woocommerce' ),
				'price-desc' => __( 'Tri par prix: Du max au min', 'woocommerce' ),
			)
		);
...
if ( ! function_exists( 'woocommerce_widget_shopping_cart_button_view_cart' ) ) {
	...
	function woocommerce_widget_shopping_cart_button_view_cart() {
		echo '<a href="' . esc_url( wc_get_cart_url() ) . '" class="button wc-forward">' . esc_html__( 'Voir panier', 'woocommerce' ) . '</a>';
	}
}
	...
	function woocommerce_checkout_payment() {
		...
		wc_get_template(
			...
			array(
				...
				'order_button_text'  => apply_filters( 'woocommerce_order_button_text', __( 'Valider la commande', 'woocommerce' ) ),
			)
		);
	}
}
		...
		switch ( $args['type'] ) {
			case 'country':
				...
				if ( 1 === count( $countries ) ) {
					...
					$field = '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="country_to_state country_select ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . implode( ' ', $custom_attributes ) . '><option value="default">' . esc_html__( 'Sélectionner un pays / région&hellip;', 'woocommerce' ) . '</option>';
...
function wc_empty_cart_message() {
	echo '<p class="cart-empty woocommerce-info">' . wp_kses_post( apply_filters( 'wc_empty_cart_message', __( 'Votre panier est vide.', 'woocommerce' ) ) ) . '</p>';
}
